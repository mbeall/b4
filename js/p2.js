...
(function( $ ){
	...
	b4.initialize = function() {
		if ( b4StoredVersion < b4CurrentVersion ) {
			// B4 upgrades on admin_init.  If it needs it, send a noop admin-ajax call to trigger.
			$.ajax( ajaxUrl, {
				data: {
					action: 'b4.noop'
				},
		...
		// ------------------------------------------------
		b4.keyboard();
		b4.comment.highlight();
		b4.utility.localizeMicroformatDates();

		// Initialize mention autocompletes if mention data exists.
		if ( mentionData )
			b4.autocomplete.mentions( $( '#posttext, #comment' ) );

		// Activate Tag Suggestions for logged users on front page
		if (isFrontPage && prologueTagsuggest && isUserLoggedIn)
			b4.autocomplete.tags( $( 'input[name="tags"]' ) );

		// Turn on automattic updating
		if (prologuePostsUpdates)
			b4.utility.toggleUpdates('unewposts');
		if (prologueCommentsUpdates)
			b4.utility.toggleUpdates('unewcomments');

		// Activate autgrow on textareas
		b4.utility.autoGrowTextarea($('#posttext, #comment'));

		// Activate tooltips on recent-comments widget
		b4.utility.tooltip($("table.b4-recent-comments a.tooltip"));

		// Hide error messages on load
		$('#posttext_error, #commenttext_error').hide();

		// Bind actions to comments and posts
		$( 'body .post, body .page' ).each( function() { b4.utility.bindActions( this, 'post' ); } );
		$( 'body .comment' ).each( function() { b4.utility.bindActions( this, 'comment' ); } );
		...
		if	($("table.b4-recent-comments").length != 0)
			lcwidget = true;
		...
		$(window).bind('beforeunload', function(e) {
			if ( $( '#posttext' ).val() || $( '#comment' ).val() ) {
				...
				if (e) { // For IE and Firefox
					e.returnValue = b4txt.unsaved_changes;
				}
				return b4txt.unsaved_changes;   // For Safari
			}
		});
				...
				$.ajax({
					...
					success: function( response ) {
						if ( 'logged_in' != response ) {
							...
							b4.utility.notification( b4txt.oops_not_logged_in + ' <a href="' + login_url + '">' + b4txt.please_log_in + '</a>.', true );
					...
					error: function() {
						...
						b4.utility.notification( b4txt.whoops_maybe_offline );
						event.preventDefault();
						return false;
					}
				});
		...
		$(window).scroll(function() { b4.utility.removeYellow(); });
	...
	b4.comment = {
				...
				if (newComments != null) {
					$.each(newComments.comments, function(i,comment) {
						...
						if (comment.widgetHtml) {
							b4.comment.insert.widget(comment.widgetHtml);
						}
						if (comment.html != '') {
							var thisParentId = 'prologue-'+comment.postID;
							b4.comment.insert.inline(thisParentId, comment.commentParent, comment.html, showNotification);
						}
					});
					if (showNotification) {
						b4.utility.notification(b4txt.n_new_comments.replace('%d', newComments.numberofnewcomments));
					}
				}
			});
			b4.utility.toggleUpdates('unewcomments');
		...
		insert: {
			...
			inline: function( postParent, comment_parent, commentHtml, showNotification ) {
				...
				newCommentsLi.slideDown( 200, function() {
					...
					newComment.children('li.newcomment').each(function() {
						if (b4.utility.isElementVisible(this) && !showNotification) {
							$(this).animate({backgroundColor:'transparent'}, {duration: 1000}, function(){
								$(this).removeClass('newcomment');
							});
						}
						b4.utility.bindActions(this, 'comment');
					});
					b4.utility.localizeMicroformatDates(newComment);
			...
			widget: function( widgetHtml ) {
				$("table.b4-recent-comments").each(function() {
					var t = $(this);
					...
					b4.utility.tooltip($("tbody tr:first td a.tooltip", t));
					if (b4.utility.isElementVisible(newCommentsElement)) {
		...
		submit: function( trigger ) {
			...
			if ('' == commenttext) {
				$("label#commenttext_error").text(b4txt.required_field).show().focus();
				return false;
			}
			...
			// If B4 thinks current user is logged in, check again for being logged in,
			// and check for a working connection to the server, to avoid hanging comments.
			if ( isUserLoggedIn && ( 'logged_in' != b4.utility.loggedInOut() ) ) {
				submitProgress.hide();
			...
			$.ajax( {
				...
				success: function( result ) {
					...
					$form.slideUp( 200, function() {
						...
						$( '#comment' ).val( '' );
						if ( errorMessage != "" )
							b4.utility.notification( errorMessage );

						b4.comment.fetch( false );
			...
			document.onkeypress = function(e) {
				...
				switch(keyCode) {
					...
					case 67:
						...
						if (!b4.utility.isElementVisible("#postbox"))
							$.scrollTo('#postbox', 50);
						$("#posttext").focus();
						if (e.preventDefault)
							e.preventDefault();
						...
						if (!commentLoop) {
							if (currPost > 0) {
								...
								if (0 != $('#'+postsOnPage[currPost]).children('ul.commentlist').length && !hidecomments) {
									...
									if (!b4.utility.isElementVisible('#'+commentsOnPost[currComment]))
										$.scrollTo('#'+commentsOnPost[currComment], 150);
									return;
								}
								if (!b4.utility.isElementVisible('#'+postsOnPage[currPost]))
					...
					case 79:
						...
						// "t" key
					case 84:
						b4.utility.jumpToTop();
						break;
					// "esc" key
					case 27:
						if (element.tagName == 'INPUT' || element.tagName == 'TEXTAREA') {
							if ( e.shiftKey && confirm( b4txt.comment_cancel_ays ) ) {
	...
	b4.plugins = {
		fluid: function( value ) {
			...
			// Sets the badge for Fluid app enclosed B4. See http://fluidapp.com/
	...
	b4.posts = {
		...
		edit: function( postId, element ) {
			...
			$.getJSON( ajaxUrl, { action: 'get_post', _inline_edit: nonce, post_ID: 'content-' + postId }, function( post ) {
				...
				if (post.post_format == 'standard' || post.post_type == 'page') {
					...
					defaultText(titleInput, b4txt.title);
				...
				b4.utility.autoGrowTextarea($(editor));
				...
				if (post.post_format == 'quote') {
					...
					defaultText(citationInput, b4txt.citation);
				...
				if ( post.post_type != 'page' ) {
					...
					defaultText(tagsInput, b4txt.tagit);
				...
				saveButton.innerHTML = b4txt.save;
				$( saveButton ).click( function() {
					var tags = ! tagsInput || tagsInput.value == b4txt.tagit ? '' : tagsInput.value;
					var args = {
						action:'save_post',
						_inline_edit: nonce,
					...
					if (post.post_format == 'standard' || post.post_type == 'page') {
						args.title = titleInput.value == b4txt.title ? '' : titleInput.value;
					} else if (post.post_format == 'quote') {
						args.citation = citationInput.value == b4txt.citation ? '' : citationInput.value;
					}
					...
					$.post(
						...
						function(result) {
							...
							$( document ).trigger( 'b4_edit_post_submit', { 'post_id' : postId, 'result' : result } );
						},
						'json');
				...
				cancelButton.innerHTML = b4txt.cancel;
				...
				// Initialize tags autocomplete
				if ( tagsInput )
					b4.autocomplete.tags( $( tagsInput ) );
			...
			ajaxCheckPosts = $.getJSON(queryString, function(newPosts){
				if (newPosts != null) {
					...
					if (!isFirstFrontPage || (typeof newPosts.html == "undefined") ) {
						...
						message = b4txt.n_new_updates.replace('%d', newUnseenUpdates) + " <a href=\"" + wpUrl +"\">"+b4txt.goto_homepage+"</a>";
						b4.utility.notification(message);
					} else {
						...
						newUpdatesLi.each(function() {
							...
							b4.utility.bindActions(this, 'post');
							if (b4.utility.isElementVisible(this) && !showNotification) {
								$(this).delay('250').animate({backgroundColor:'transparent'}, 2500, function() {
									$(this).removeClass('newupdates');
									b4.utility.titleCount();
								});
							}
							b4.utility.localizeMicroformatDates(this);
							counter++;
						});
						if (counter >= newPosts.numberofnewposts && showNotification) {
							var updatemsg = b4.utility.isElementVisible('#main > ul >li:first') ? "" :  "<a href=\"#\"  onclick=\"b4.utility.jumpToTop();\" \">"+b4txt.jump_to_top+"</a>" ;
							b4.utility.notification(b4txt.n_new_updates.replace('%d', counter) + " " + updatemsg);
							b4.utility.titleCount();
						}
					}
					$('.newupdates > h4, .newupdates > div').hover( b4.utility.removeYellow, b4.utility.removeYellow );
				}
			});
		...
		submit: function( trigger ) {
			...
			if ("" == posttext) {
				$("label#posttext_error").text(b4txt.required_field).show().focus();
				return false;
			}
			...
			$.ajax({
				...
				success: function(result) {
					if ("0" == result)
						errorMessage = b4txt.not_posted_error;

					$('#posttext').val('');
					$('#posttitle').val('');
					$('#postcitation').val('');
					$('#tags').val(b4txt.tagit);
					if(errorMessage != '')
						b4.utility.notification(errorMessage);
					...
					if (isFirstFrontPage && result != "0") {
						b4.posts.fetch(false);
					} else if (!isFirstFrontPage && result != "0") {
						b4.utility.notification(b4txt.update_posted);
					}
					...
					$( document ).trigger( 'b4_new_post_submit_success', { 'post_id' : result } );
				  },
				error: function(XMLHttpRequest, textStatus, errorThrown) {
					...
					$( document ).trigger( 'b4_new_post_submit_error', { 'request' : XMLHttpRequest, 'status' : textStatus, 'error' : errorThrown } );
				},
				timeout: 60000
			});
	...
	b4.utility = {
		autoGrowTextarea: function( textareas ) {
			...
			textareas.on('focus blur keydown paste', function(e) {
				...
				if ( 'paste' === e.type )
					setTimeout( function() { b4.utility.expandTextarea( t, $('#' + $(t).prop('id')).prop('scrollHeight') ); }, 30 );
				else
					b4.utility.expandTextarea( t, scrollHeight );
			});
		},
		bindActions: function( element, type ) {
			...
			switch (type) {
				case "comment" :
					...
					$(element).hover( b4.utility.removeYellow, b4.utility.removeYellow );
					if (inlineEditComments != 0 && isUserLoggedIn) {
						...
						thisCommentEditArea.editable(ajaxUrl, {event: 'edit', loadurl: ajaxUrl + '&action=get_comment&_inline_edit=' + nonce,
							...
							indicator : commentEditSpinnerText, loadtext: b4txt.loading, cancel: b4txt.cancel,
							submit  : b4txt.save, tooltip   : '', width: '90%', onblur: 'ignore',
		...
		localizeMicroformatDates: function( scopeElem ) {
			(scopeElem? $('abbr', scopeElem) : $('abbr')).each(function() {
				...
				if ( date && date > 0 )
					t.html(b4txt.date_time_format.replace('%1$s', locale.date(b4txt.time_format, date)).replace('%2$s', locale.date(b4txt.date_format, date)));
			});
		},
		...
		removeYellow: function() {
			$('li.newcomment, tr.newcomment').each(function() {
				if (b4.utility.isElementVisible(this)) {
		...
		toggleUpdates: function( updater ) {
			switch (updater) {
				case "unewposts":
					if (0 == getPostsUpdate) {
						getPostsUpdate = setInterval(b4.posts.fetch, updateRate);
					}
					else {
				...
				case "unewcomments":
					if (0 == getCommentsUpdate) {
						getCommentsUpdate = setInterval(b4.comment.fetch, updateRate);
					}
					else {
	...
	if ((inlineEditPosts || inlineEditComments ) && isUserLoggedIn) {
		$.editable.addInputType('autogrow', {
			...
			plugin : function(settings, original) {
				window.b4.utility.autoGrowTextarea($('textarea', this));
			}
		});
	}
