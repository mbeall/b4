 ...
 * @package B4
 */
class B4_Recent_Tags extends WP_Widget {
	function __construct() {
		parent::__construct( false, __( 'B4 Recent Tags', 'b4' ), array( 'description' => __( 'The tags from the latest posts.', 'b4' )));
...
?>
	...
	<p>
		<label for="<?php echo $num_to_show_id ?>"><?php _e( 'Number of tags to show:', 'b4' ); ?>
...
<?php
	...
	function widget( $args, $instance ) {
		...
		$title = (isset( $instance['title'] ) && $instance['title'])? $instance['title'] : __( 'Recent tags', 'b4' );
	...
	function recent_tags( $num_to_show ) {
		$cache = wp_cache_get( 'b4_recent_tags', 'widget' );
		if ( !is_array( $cache ) ) {
			$cache = array();
		}
		...
		wp_cache_add( 'b4_recent_tags', $cache, 'widget' );
		return $recent_tags;
	}

}

register_widget( 'B4_Recent_Tags' );
