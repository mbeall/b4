 ...
 * @package B4
 */
class B4_Recent_Comments extends WP_Widget {
	function __construct() {
		parent::__construct( false, __( 'B4 Recent Comments', 'b4' ), array( 'description' => __( 'Recent comments with avatars.', 'b4' ) ) );
	...
	function flush_widget_cache() {
		wp_cache_delete( 'b4_recent_comments', 'widget' );
	}
...
<?php
	...
	function widget( $args, $instance ) {
		...
		$title = (isset( $instance['title'] ) && $instance['title'])? $instance['title'] : __( 'Recent comments', 'b4' );
...
?>
		<table class="b4-recent-comments" cellspacing="0" cellpadding="0" border="0">
	...
	static function comment_url_maybe_local( $comment ) {
		...
		// Works by detecting if the comment's post is visible on the page... may break if B4 decides to do clever stuff with comments when paginated
		$comment_url = get_comment_link( $comment );

		if ( '1' === get_option( 'b4_hide_threads' ) )
			return $comment_url;
	...
	function recent_comments( $num_to_show ) {
		global $wpdb;
		$cache = wp_cache_get( 'b4_recent_comments', 'widget' );
		if ( !is_array( $cache ) ) {
			$cache = array();
		}
		...
		wp_cache_add( 'b4_recent_comments', $cache, 'widget' );
		return $comments;
	}
	...
	static function single_comment_html($comment, $avatar_size ) {
		...
		if ( !$comment->comment_author ) $comment->comment_author = __( 'Anonymous', 'b4' );
		...
		$comment_url = B4_Recent_Comments::comment_url_maybe_local( $comment );

		$row .= sprintf( '<td class="text">'.__( "%s on <a href='%s' class='tooltip' title='%s'>%s</a>" , 'b4' ) . '</td></tr>', $author_html, esc_url( $comment_url ), esc_attr($comment_content), $post_title );
		return $row;
	}
}

register_widget( 'B4_Recent_Comments' );
