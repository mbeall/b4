...
if ( ! class_exists( 'B4Ajax_Read' ) ) {
	require dirname( __FILE__ ) . '/ajax-read.php';
}

if ( defined('DOING_AJAX') && DOING_AJAX && isset( $_REQUEST['b4ajax'] ) ) {
	add_action( 'admin_init', array( 'B4Ajax', 'dispatch' ) );
}

/*
 * We include all of B4Ajax_Read's methods so that the old, deprecated API (db_version=1) still works for logged in users.
 ...
 * We only include in now for backward compatibility so that currently open B4 windows continue to function.
 * By the time the next release rolls around, it'll be safe to remove.
 */
class B4Ajax extends B4Ajax_Read {
	static function dispatch() {
		$action = isset( $_REQUEST['action'] ) ? $_REQUEST['action'] : '';

		do_action( "b4_ajax", $action );
		if ( is_callable( array( 'B4Ajax', $action ) ) )
			call_user_func( array( 'B4Ajax', $action ) );
		else
			die( '-1' );
		exit;
	}
	...
	static function get_post() {
		...
		if ( !is_user_logged_in() ) {
			die( '<p>'.__( 'Error: not logged in.', 'b4' ).'</p>' );
		}
		$post_id = $_GET['post_ID'];
		$post_id = substr( $post_id, strpos( $post_id, '-' ) + 1 );
		if ( !current_user_can( 'edit_post', $post_id ) ) {
			die( '<p>'.__( 'Error: not allowed to edit post.', 'b4' ).'</p>' );
		}
		...
		$post_format = b4_get_post_format( $post_id );
	...
	static function get_comment() {
		...
		echo apply_filters( 'b4_get_comment_content', $comment->comment_content, $comment_id );
	}
	...
	static function save_post() {
		...
		if ( ! empty( $new_post_title ) )
			...
			$post_title = b4_title_from_content( $new_post_content );
		...
		if ( 'quote' == $post_format )
			$content = apply_filters( 'b4_get_quote_content', $post->post_content );
		else
		...
		echo json_encode( array(
			'title' => $post->post_title,
			'content' => $content,
			'tags' => get_tags_with_count( $post, '', __( '<br />Tags:' , 'b4' ) . ' ', ', ', ' &nbsp;' ),
		) );
	}
	...
	static function save_comment() {
		...
		if ( !current_user_can( 'edit_post', $comment->comment_post_ID ) ) {
			die( '<p>'.__( 'Error: not allowed to edit this comment.', 'b4' ).'</p>' );
		}
	...
	static function new_post() {
		...
		if ( ! ( current_user_can( 'publish_posts' ) ||
		        (get_option( 'b4_allow_users_publish' ) && $user_ID )) ) {

			die( '<p>'.__( 'Error: not allowed to post.', 'b4' ).'</p>' );
		}
		...
		if ( __( 'Tag it', 'b4' ) == $tags )
			$tags = '';

		// For empty or placeholder text, create a nice title based on content
		if ( empty( $title ) || __( 'Post Title', 'b4' ) == $title )
	    	$post_title = b4_title_from_content( $post_content );
		else
			$post_title = $title;

		$post_format = 'status';
		$accepted_post_formats = apply_filters( 'b4_accepted_post_cats', b4_get_supported_post_formats() ); // Keep 'b4_accepted_post_cats' filter for back compat (since B4 1.3.4)
